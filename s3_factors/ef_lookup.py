...
from s3_factors.material_mapping import material_mapping
...
def _load_ceda() -> tuple[list[str], list[str], dict[str, dict[str, float]]]:
    ...
    sector_names: list[str] = [str(v) for v in df.iloc[2, 3:].tolist() if pd.notna(v)]
    sector_codes: list[str] = [str(v) for v in df.iloc[3, 3:].tolist() if pd.notna(v)]
...
def lookup_ef(
    ...
    sector_code, sector_name, confidence, alternatives = _find_sector(
        material, sector_names, sector_codes
    )
...
def search_sectors(q: str | None = None, *, limit: int = 50) -> list[tuple[str, str]]:
    ...
    if q and q.strip():
        query = q.strip().lower()
        pairs = [
            (code, name) for code, name in pairs if query in name.lower() or query in code.lower()
        ]
    return pairs[:limit]
