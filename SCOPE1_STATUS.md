...
- ~~RLS-hard role enforcement~~ ✅ (migration 116, §0c). ~~ESRS/CDP/GHGRP exports~~ ✅ done — see §0d.

## 0d. Multi-regime disclosure exports (ESRS E1 / CDP / EPA GHGRP)
Added three disclosure formats (PDF + XLSX each) beyond SB 253. Architecture = one neutral IR + one renderer pair + small per-regime mappers (`s1_reporting/disclosures/{ir,render,mappers}.py`): `DisclosureData + DisclosureMeta → Disclosure (sections) → render_pdf/render_xlsx`. Adding a regime = one ~30-line mapper.
- **🔴 Correctness fix shipped:** the report/exports were **combustion-only** — fugitive + process were summed only in the dashboard frontend, so **SB 253 under-reported**. New `DisclosureData` (in `report.py`) *composes* the combustion `InventoryReport` with fugitive + process (leaves `InventoryReport.total_scope1_tco2e` unchanged/combustion-only to avoid breaking dashboard/trends/recalc). `_assemble_disclosure_data` fetches all three; **SB 253 gross now = combustion + fugitive + process** (routes fixed too).
- Regimes: ESRS E1-6 (gross + boundary + by-gas + biogenic), CDP C6/C7 (gross + by-gas + by-facility + verification status), GHGRP Subpart C (per-facility×per-fuel CO₂/CH₄/N₂O **kg** + calc tier + >25,000 tCO₂e threshold flag). `build_pdf/build_xlsx` kept back-compat (accept bare `InventoryReport` → fugitive/process=0).
- Routes: `GET /inventories/{id}/report/{regime}.pdf|.xlsx` (regime ∈ esrs-e1|cdp|epa-ghgrp; 422 unknown). Frontend: `downloadDisclosure` + 6 buttons on `/scope-1/report`. Added `fuel`+`ef_tier` to `ReportRecord` (for GHGRP). **No migration.** 609 tests (9 new). No DB changes to apply.
