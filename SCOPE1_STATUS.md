...
- **Process emissions** — `4a3d90d` (**migration 114**, `s1_process_record` table, band 110–199; completes the direct-emissions taxonomy = combustion + fugitive + process. Pure `s1_process`: IPCC-default EF library [cement clinker/lime/glass/soda ash/ammonia→CO2; nitric/adipic acid→N2O] + custom; emission = activity×EF; stores emitted **gas mass (kg)**, derives tCO2e via `s1_calc.gwp` at the AR version [N2O processes track AR5/AR6]. `GET /process-factors`, `POST /process`, `GET /inventories/{id}/process?ar_version=`, `DELETE`. `/scope-1/process` page; dashboard gross = combustion + fugitive + process). **Migration 114 NOT yet applied — needs dev + prod.** (Gotcha hit: forgot to import the request DTO into scope1.py → with `from __future__ import annotations`, FastAPI silently treated the body param as a query param → 422 "field required: query.req". Always import request models.)

Files: `s1_calc/ s1_factors/ s1_consolidation/ s1_reporting/ s1_intake/{,ocr,bayou} s1_onboarding/ s1_recalc/ s1_fugitive/ s1_process/`, `api/routes/scope1.py`, `db/scope1_store.py`, `api/models/scope1_schemas.py`, `api/graphs/scope1_ocr_graph.py`, `scripts/seed_scope1_reference.py`, `supabase/migrations/03[0-9]_* + 110–114`, `frontend/src/app/scope-1/*`, `frontend/src/lib/scope1-api.ts`, `tests/test_s1_*.py`.
...
- **Backend tests:** `cd $WT && PYTHONPATH=$SCRATCH/exportlibs ANTHROPIC_API_KEY="" $ORIG/.venv/bin/python -m pytest tests -q` (427 passing).
...
- **⚠ Apply migrations 110–114** to the dev DB (and prod at release), like 030–039. 110 (`s1_ef_override`): EF override reads/writes 500 until applied; intake falls back to EPA default. 111 (`s1_inventory` metric cols): trends totals/YoY work but intensity null + `POST /metrics` 500. 112 (`s1_base_year_recalc_event`): recalc endpoints 500. 113 (`s1_fugitive_record`): fugitive endpoints 500. 114 (`s1_process_record`): process endpoints 500. Dashboard fugitive/process fetches are guarded (`.catch(()=>null)`) → dashboard still loads if 113/114 unapplied. Band 110–199 open for future s1 schema.
- **Next options (post-MVP, user's pick):** RLS-hard role enforcement (close the app-layer-only gap), more disclosure exports (ESRS E1/CDP/GHGRP), or the parked Bayou lane (6/7). ~~base-year recalc~~ ✅, ~~fugitive/refrigerant~~ ✅, ~~process emissions~~ ✅ done.
...
- **V1 (explicitly out of MVP):** ~~base-year recalculation engine~~ ✅ (49f00a4), ~~refrigerant/fugitive~~ ✅ (f340c02), ~~process emissions~~ ✅ (4a3d90d), Samsara live telematics, ESRS/CDP/GHGRP exports, Scope 2 bolt-on, RLS-hard role enforcement, SOC 2, email display in the roles UI (currently truncated `user_id`).
